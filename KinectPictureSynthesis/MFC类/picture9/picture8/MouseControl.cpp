...
int CMouseControl::NearestBody(IBody* pBody[])
{
	Joint joint[BODY_COUNT][JointType_Count];
	float spineBases[BODY_COUNT] = { 0 };
	for (int i = 0; i < BODY_COUNT; i++)
	{
		pBody[i]->GetJoints(JointType::JointType_Count, joint[i]);
		spineBases[i] = joint[i][JointType::JointType_SpineBase].Position.Z;
	}
	int nearest = 0;
	for (int i = 0; i < BODY_COUNT; i++)
	{
		if (spineBases[nearest] <= spineBases[i])
		{
			nearest = nearest;
		}
		else
